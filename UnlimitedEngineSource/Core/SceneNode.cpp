...
unsigned int SceneNode::attachChild( Ptr child )
{
    child->mParent = this;
...
SceneNode::Ptr SceneNode::detachChild( const unsigned int index )
{
    assert( mChildren.size() > index );
    Ptr result = std::move( mChildren.at( index ) );
