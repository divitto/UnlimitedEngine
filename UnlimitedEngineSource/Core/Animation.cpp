...
namespace
{
    const auto Table = initializeAnimationData;
}
...
, mFrameSize( Table[mType].frameSize )
, mNumFrames( Table[mType].numFrames )
, mCurrentFrame( 0 )
, mDuration( Table[mType].duration )
, mElapsedTime( sf::Time::Zero )
, mRepeat( Table[mType].isRepeated )
{
}
